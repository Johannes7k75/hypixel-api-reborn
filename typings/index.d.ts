...
export interface clientOptions {
	...
	cacheFilter?:
		| string
		| string[]
		| { whitelist: string | string[]; blacklist: string | string[] };
...
declare module "hypixel-api-reborn" {
	...
	const Utils: {
		...
		guildExp: {
			parseHistory(
				historyDate: Record<string, unknown>,
			): { day: string; date: Date; exp: number; totalExp: number }[];
	...
	class Client extends EventEmitter {
		...
		getSkyblockMember(
			query: string,
			options?: skyblockMemberOptions,
		): Promise<Map<string, SkyblockMember>>;
		/**
		 * @description Allows you to get statistics of player
		 * @param {string} query - player nickname or uuid
		...
		on(
			...
			listener: (url: string, options: Record<string, unknown>) => void,
		): this;
